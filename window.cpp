...
Window::Window(const char *title, uint32_t width, uint32_t height,
  ...
  bool success = SDL_CreateWindowAndRenderer(
      title, width, height, SDL_WINDOW_RESIZABLE, &m_window, &m_renderer);
  if (!success) {
  ...
  if (format) {
    m_format = format;
    m_surf = SDL_CreateTexture(m_renderer, format, SDL_TEXTUREACCESS_STREAMING,
                               width, height);
    if (!m_surf) {
...
std::tuple<unsigned char *, unsigned, unsigned> Window::get_surf() {
